...
export function VoiceRecorder({
  ...
  const startRecording = async () => {
    try {
      ...
      console.log(
        "[v0] Microphone access granted, stream active:",
        stream.active
      );
      ...
      mediaRecorder.ondataavailable = (event) => {
        console.log(
          "[v0] ondataavailable fired, data size:",
          event.data.size,
          "state:",
          mediaRecorder.state
        );
        if (event.data.size > 0) {
          ...
          console.log(
            "[v0] Audio chunk added, total chunks:",
      ...
      setTimeout(() => {
        console.log(
          "[v0] MediaRecorder state after start:",
          mediaRecorder.state
        );
      ...
      transcriptionIntervalRef.current = setInterval(async () => {
        ...
        const audioBlob = new Blob(chunksToTranscribe, {
      ...
      imageGenerationIntervalRef.current = setInterval(() => {
        ...
        console.log(
          "[v0] Image generation check - transcript length:",
          currentText.length
        );
  ...
  const transcribeAudio = async (audioBlob: Blob) => {
    ...
    try {
      ...
      if (newText.trim()) {
        ...
        const updatedTranscription = currentText
          ? currentText + " " + newText
          : newText;
      ...
      <div className="flex-1 min-h-[120px] lg:min-h-[200px] p-4 rounded-lg border overflow-y-auto bg-background shadow-none">
        {transcription ? (
          ...
          <p className="text-sm text-muted-foreground">
            {isRecording
              ? "Listening... Start speaking to generate images"
        ...
        <p className="text-xs text-muted-foreground text-center">
          {credits === null ? (
            ...
            <span className="text-destructive font-semibold">
              No credits remaining • Add API key to continue
            </span>
          )}
        </p>
      </div>
